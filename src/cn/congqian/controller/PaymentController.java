...
public class PaymentController extends HttpServlet {
	 ...
	 * @throws IOException
	 */
	private void QRCode2(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		HttpSession session = req.getSession(true);
		String uuid = UUID.randomUUID().toString();
		session.setAttribute("orderuuid", uuid);
		Goods goods = (Goods) session.getAttribute("goods2");
		String parameter = "欢迎购买卡通玩具，您购买了"+goods.getGoods_name();
		parameter+= "感谢您的支付您需支付："
				+  req.getParameter("pieces")  + "元"
				+ "付款id："+uuid;
		System.out.println(parameter);
